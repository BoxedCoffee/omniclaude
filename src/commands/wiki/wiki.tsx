...
Manage the OmniClaude project wiki stored in .openclaude/wiki.
...
function formatInitResult(result: Awaited<ReturnType<typeof initializeWiki>>): string {
  const lines = [`Initialized OmniClaude wiki at ${result.root}`]
...
function formatStatus(status: Awaited<ReturnType<typeof getWikiStatus>>): string {
  if (!status.initialized) {
    return `OmniClaude wiki is not initialized in this project.\n\nRun /wiki init to create ${status.root}.`
  }

  return [
    'OmniClaude wiki status',
    '',
    `Root: ${status.root}`,
...
function formatIngestResult(
  ...
  return [
    `Ingested ${result.sourceFile} into the OmniClaude wiki.`,
    '',
    `Title: ${result.title}`,
