# OmniClaude on Android (Termux)

A complete guide to running OmniClaude on Android using Termux + proot Ubuntu.
...
OmniClaude requires [Bun](https://bun.sh) to build, and Bun does not support Android natively. The workaround is running a real Ubuntu environment inside Termux via `proot-distro`, where Bun's Linux binary works correctly.
...
### Step 6 — Build OmniClaude

```bash
cd /data/data/com.termux/files/home/omniclaude
bun run build
```

You should see:
```
✓ Built omniclaude v0.1.6 → dist/cli.mjs
```
...
Both were tested and fail due to OmniClaude's large system prompt (~50K tokens):
...
- Run `bun run build` again only if you pull updates to the OmniClaude repo.
